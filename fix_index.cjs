...
// Remove all instances of the book section
const sectionRegex = /<!-- Aura Learning Official Guide Section -->[\s\S]*?(?=<!-- Features & Showcase Section -->)/g;
html = html.replace(sectionRegex, '');

// Fix any h1/h2 messes if possible, but since we didn't use git, let's see. 
// Actually I ran sed to replace ALL </h2> with </h1>! I need to fix that.
// Let's replace </h1> back to </h2> where the class matches the sub-sections.
// In the original, the hero section has </h1>, and other sections have </h2>.
html = html.replace(/<h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight">(.*?)<\/h1>/g, '<h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight">$1</h2>');

html = html.replace(/<h1 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">(.*?)<\/h1>/g, '<h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">$1</h3>');

// It's probably safer to just pull the original from supabase or something? No, I don't have it.
// Let's carefully re-add the book section.
const bookSection = `
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-slate-50 dark:bg-slate-900/60 rounded-[2.5rem] p-8 sm:p-12 border border-slate-200/50 dark:border-slate-800 shadow-xl dark:shadow-2xl flex flex-col md:flex-row items-center gap-10 md:gap-16 transition-colors duration-300">
                ...
                <div class="w-full md:w-3/5 flex flex-col items-center md:items-start text-center md:text-left">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white mb-3 tracking-tight">
...
html = html.replace(insertMarker, bookSection + "\n    " + insertMarker);

// Apply meta tags again
html = html.replace(
    /<title>.*?<\/title>/s,
